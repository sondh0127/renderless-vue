...
function handleKeyUp(event: KeyboardEvent) {
  ...
  switch (event.key) {
    case Keys.Tab:
      ...
      // Check if the last focused element is *after* the button in the DOM
      const focusableElements = getFocusableElements()
      const previousIdx = focusableElements.indexOf(previousActiveElementRef.value as HTMLElement)
      const buttonIdx = focusableElements.indexOf(dom(api.button)!)
      if (buttonIdx > previousIdx) return
...
const propsWeControl = isWithinPanel
  ...
  : {
      ...props,
      id: api.buttonId,
      'aria-expanded': props.disabled ? undefined : api.popoverState.value === PopoverStates.Open,
...
<template>
  <button ref="elementRef" type="button" v-bind="propsWeControl">
    <slot :open="open"></slot>
  </button>
</template>
